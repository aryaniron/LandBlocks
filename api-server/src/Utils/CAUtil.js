...
export const registerAndEnrollUser = async (
  ...
  try {
    ...
    if (!adminIdentity) {
      ...
      throw "An identity for the admin user does not exist in the wallet";
    }
    ...
    return `Failed to register user : ${error}`;
  }
};

export const isUserRegistered = async (wallet, userId) => {
  try {
    // Check to see if we've already enrolled the user
    const userIdentity = await wallet.get(userId);
    if (userIdentity) {
